...
Imagine you’re working on **Prac-2.2** and, by mistake, you break something in your **Prac-2.1** code.
If the **Prac-2.2** tests don’t catch that error, you’ll never know, because you’re only running the `prac_2_2` tests.
And since you haven’t implemented **Prac-2.3** yet, you can’t just run *all* the tests at once.

**How do we avoid this problem?**

The best solution is that **every time you push code to GitHub, all the test suites you’ve completed should run automatically**.
That way, if you accidentally break an earlier task, you’ll know immediately (for example, by receiving a notification email which GitHub sends by default with Actions).

This is exactly what **GitHub Actions** is for.

The teachers have already set everything up for you (you’re welcome! 😉).
Your only job is: whenever you’ve finished a task and you’re confident it works, **uncomment the corresponding test section** in `.github/workflows/test.yml`.

For example, to make the **Prac-2.1** tests run automatically every time you push code, just uncomment its section in the file, like this:
