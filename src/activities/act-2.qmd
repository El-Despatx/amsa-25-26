...
You may have heard about the linux program [htop](https://htop.dev/){target="_blank"}, it is an **interactive process viewer** for Unix-like systems.
...
*Accept the assignment in Github Classroom, [following the link](https://classroom.github.com/a/BwZznF7h){target="_blank"}*
...
A python library available on [PyPI](https://pypi.org/project/amsatop/){target="_blank"} has been built with all of our love so that it already implements the UI and some abstractions. Make sure to check out the [docs](https://docs.amsa.lol/){target="_blank"}!

Building your solution on top of a library makes it easier to give you useful tools such as tests and [helper functions](https://docs.amsa.lol/_apidoc/amsatop.utils.html){target="_blank"}.
...
Implement [the get_processes method](https://github.com/El-Despatx/amsatop-template/blob/main/src/myhtop/amsatop_solution.py#L11){target="_blank"} so it returns a list where each element is an instance of [`Process`](https://docs.amsa.lol/_apidoc/amsatop.html#amsatop.Process){target="_blank"}. There has to be an element on the list for each **process, thread and kthread** running on the system.
...
::: callout-tip
Make sure to read through [the docs](https://docs.amsa.lol/_apidoc/amsatop.utils.html){target="_blank"} of the helper classes and functions, we promise using them will make your life easier 😉!
:::

::: {.callout-important title="**Very very important 🚨**"}
It is **very important** that you make use of the property [proc_folder](https://docs.amsa.lol/_apidoc/amsatop.html#amsatop.Htop.proc_folder){target="_blank"} of the [Htop](https://docs.amsa.lol/_apidoc/amsatop.html#amsatop.Htop){target="_blank"} class when accessing files on `"/proc"`. **Do not use a hardcoded string "/proc"**, instead use `self.proc_folder` (which is a string that defaults to "/proc"). This is essential for the tests to work.
...
As stated on the [week-2 slides](weeks/week-2){target="_blank"}, there are different ways to get information about /proc. We recommend you to use the following ones:
...
- To detect processes or kernel threads, the algorithm is "trivial", the idea is:
    ...
    2) Now you have to filter if they're a process o kernel thread. We recommend you to [apply the mask](https://www.youtube.com/watch?v=AHiepE54l88){target="_blank"} that the [htop code applies to the flags content](weeks/week-2.html#/properties){target="_blank"} to see if they are a kernel thread [^1].
...
Implement [get_priorities method](https://github.com/El-Despatx/amsatop-template/blob/main/src/myhtop/amsatop_solution.py#L14){target="_blank"} so it returns a list of [Process](https://docs.amsa.lol/_apidoc/amsatop.html#amsatop.Process){target="_blank"}. It has to be the same list we obtained on **Prac-2.1** but now we want you to set the value of the `priority` field if available.
...
Keep in mind that [each signal corresponds to a number...](https://faculty.cs.niu.edu/~hutchins/csci480/signals.htm){target="_blank"}
...
Remember that, once you entered the Github Classroom on [section 2](act-2.qmd#delivery){target="_blank"}, you will have to clone your repository with [^2]:
...
[^2]: If you don't have any idea of what we're talking about, revisit the [setting up the VM and brief git summary](/resources/vm_git.qmd){target="_blank"}
...
::: callout-tip
Keep in mind that the first two items (tests and linter) can be automatically verified using [Github Actions on your repository](/resources/gh_action.qmd){target="_blank"}
:::
...
3.  The process information needed can **only be obtained by reading the contents of `/proc` and parsing it out** (remember you have [helpers](https://docs.amsa.lol/_apidoc/amsatop.utils.html){target="_blank"} for that).
4.  Make sure the tests are passing on the [Ubuntu VM](https://amsa.lol/#resources){target="_blank"}, we'll pass them on the VM too.
