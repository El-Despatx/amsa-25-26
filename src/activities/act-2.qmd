...
The mask to detect if the Signal N is ignored, has the following formula:

> mask(N) = 2^(N-1) -> Where N is the signal number.

So:

- If I want to check if the process ignores the signal number 1, I can apply the (& 1) mask
- If I want to check if the process ignores the signal number 2, I can apply the (&2) mask.
- If I want to check if the process ignores the signal number 4, I can apply the (&4) mask.
- Etc.

Be aware that if the result of SIG_IGN & mask:

- Is zero: Means that the signal is not ignored
- Is different of zero: Means that the signal is ignored.
...
::: callout-tip
Learn more about [pytest options here](/resources/pytest.qmd)
:::
...
-   [Pytest for begginers](/resources/pytest.qmd)
-   [Executing Github Actions on your repository for knowing your mark](/resources/gh_action.qmd)
