...
*Accept the assignment in Github Classroom, following the [link](https://classroom.github.com/a/eoMHxDDl)*
...
### Installing libfuse
We use a Linux feature called FUSE in our tests. This lets us emulate the special capabilities of the /proc filesystem without burdening you (the student) with the technical details of our test setup.
...
### [Optional] Improving tests experiences with Github Actions

Imagine you accidentally break part of your code from Prac-2.1 while working on Prac-2.2. If this issue isn’t detected by the Prac-2.2 test suite, you won’t realize something is broken, since you’ll only be running the prac_2_2 tests, and you can’t yet run all at onece because you didn't implemnet prac_2_3 yet.

So, how can we solve this?

Ideally, every time we push code to the repository, all the relevant test suites should run automatically. If any of them fail because we broke something, we should immediately be notified (for example, by email).

This is exactly the kind of workflow that GitHub Actions can automate for us.

The teachers have already set everything up for you (you’re welcome! 😉). The only thing you need to do is, once you’ve confirmed that your `prac_2_x` works, uncomment the corresponding test section in `.github/workflows/test.yml` in your repository.

For example, if you want the `prac_2_1` tests to run automatically whenever you push to your repository, simply uncomment its section, like this:
...
```{.yaml code-line-numbers="true"}
       ...
       # - name: "Run pytest on prac 2.1"
       #   run: uv run pytest test/prac_2_1/
```
::::
